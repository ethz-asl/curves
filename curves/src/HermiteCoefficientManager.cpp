...
void HermiteCoefficientManager::insertCoefficients(
    const std::vector<Time>& times, const std::vector<Coefficient>& values,
...
bool HermiteCoefficientManager::hasCoefficientAtTime(Time time) const {
  std::map<Time, KeyCoefficientTime*>::const_iterator it = timeToCoefficient_
      .find(time);
...
bool HermiteCoefficientManager::hasCoefficientWithKey(Key key) const {
  boost::unordered_map<Key, KeyCoefficientTime>::const_iterator it =
      coefficients_.find(key);
...
/// with this key.
void HermiteCoefficientManager::setCoefficientByKey(
    Key key, const Coefficient& coefficient) {
  boost::unordered_map<Key, KeyCoefficientTime>::iterator it = coefficients_
      .find(key);
  CHECK( it != coefficients_.end() ) << "Key " << key
...
/// This function fails if there is no coefficient associated
/// with this key.
void HermiteCoefficientManager::setCoefficientVectorByKey(
    Key key, const Eigen::VectorXd& vector) {
  boost::unordered_map<Key, KeyCoefficientTime>::iterator it = coefficients_
      .find(key);
  CHECK( it != coefficients_.end() ) << "Key " << key
      << " is not in the container.";
...
bool HermiteCoefficientManager::getCoefficientsAt(
    Time time,
...
void HermiteCoefficientManager::getCoefficients(
...
void HermiteCoefficientManager::checkInternalConsistency(bool doExit) const {
  ...
  for (; it != timeToCoefficient_.end(); ++it) {
    ...
    boost::unordered_map<Key, KeyCoefficientTime>::const_iterator itc =
        coefficients_.find(key);
    CHECK( itc != coefficients_.end() ) << "Key " << key
        << " is not in the map";
